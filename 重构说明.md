...
5. ✅ 使用密封类表示 UI 状态
6. ✅ 优化生命周期感知
7. ✅ 提供 ViewBinding 和 DataBinding 双实现
...
**当前问题：** ViewModel 在 `init` 中自动加载数据，可能导致并发请求。
            ...
            userRepository.getUsers()
                ...
                .collect { result ->
                    result.fold(
                        ...
                        onFailure = { exception ->
                            _uiState.value = UserUiState.Error(exception.message ?: "加载失败")
                        }
                    )
                }
        } finally {
...
**当前问题：** Repository 先发送缓存数据，再发送网络数据，可能导致 UI 闪烁。
...
```kotlin
// 使用 distinctUntilChanged 避免重复发送相同数据
...
本项目同时提供了 ViewBinding 和 DataBinding 两种实现：
...
**DataBinding 实现（推荐用于 MVVM）:**
- `UserViewModelDataBinding.kt`
- `UserFragmentDataBinding.kt`
- `UserAdapterDataBinding.kt`
- `fragment_user_databinding.xml`

### 为什么 DataBinding 更适合 MVVM？

1. **真正的数据驱动**: XML 中直接绑定 ViewModel，UI 自动更新
2. **减少样板代码**: 不需要手动 observe 和更新 UI
3. **双向绑定**: 支持 `@={}` 语法
4. **更符合架构理念**: View 完全被动，只负责显示

详细对比和使用说明请查看 [DataBinding使用说明.md](./DataBinding使用说明.md)
...
### 4. 迁移到 Jetpack Compose

考虑使用 Jetpack Compose 替代 XML 布局，这是 Android UI 的未来趋势。
...
- [Android 单元测试](https://developer.android.com/training/testing/unit-testing)
